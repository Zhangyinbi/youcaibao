...
public class LGImgCompressor {
    ...
    public String compressImage(String srcImageUri, int outWidth, int outHeight, int maxFileSize) {
        ...
        while (baosLength / 1024 > maxFileSize) {//循环判断如果压缩后图片是否大于maxMemmorrySize,大于继续压缩
            ...
            options_ = Math.max(0, options_ - 5);//图片质量每次减少 5
    ...
    private static String getOutputFileName(String srcFilePath) {
        ...
        File file = new File(Environment.getExternalStorageDirectory().getPath(), "designer");
        if (!file.exists()) {
            file.mkdirs();
        }
    ...
    public static String  getOutputFilePath(String srcFilePath){
        return getOutputFileName(srcFilePath);
    }
